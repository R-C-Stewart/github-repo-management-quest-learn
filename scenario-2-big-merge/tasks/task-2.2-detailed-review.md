...
1. **Open VS Code with PR content:**
   ```bash
   cd scenario-2-big-merge/pr-content/changed-files
   code .
   ```
...
### 1. Review High-Priority File: API Reference Changes

**GitHub Copilot Chat Prompt:**

```
@workspace I'm reviewing docs/api-reference.md which has major changes (+123, -67 lines).

[Open and share the modified api-reference.md file]

Perform a detailed review checking:

1. **Technical Accuracy:**
   - Are API endpoints correct?
   - Are request/response examples valid?
...
3. **Consistency:**
   - Is the format consistent across all endpoints?
...
4. **Code Examples:**
   ...
   - Would they actually work if copy-pasted?
...
```
@workspace Review the new tutorial file: tutorials/advanced-integrations.md
...
2. **Content Quality:**
   ...
   - Are explanations thorough but concise?
...
5. **Practical Value:**
   - Would a developer successfully complete this tutorial?
...
```
@workspace This PR modifies several files and deletes guides/legacy-setup.md.
...
3. Check if any links point to the deleted file
4. Verify anchor links (#sections) match actual headings
5. Check if navigation/TOC files reference deleted content

For the deleted file:
@workspace Search the entire repository for any references to "legacy-setup" - are there broken links we need to fix?
